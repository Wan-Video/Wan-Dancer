...
def rope_apply_usp(x, freqs, num_heads):
    x = rearrange(x, "b s (n d) -> b s n d", n=num_heads)
...
class SelfAttentionUSP(nn.Module):
    def __init__(self, dim: int, num_heads: int, eps: float = 1e-6):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        self.attn = LongContextAttention()
    ...
    def forward(self, x, freqs):
        ...
        q = rope_apply_usp(q, freqs, self.num_heads)
        k = rope_apply_usp(k, freqs, self.num_heads)
...
class DiTBlock(nn.Module):
    def __init__(self, has_image_input: bool, dim: int, num_heads: int, ffn_dim: int, eps: float = 1e-6, use_usp=False):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        if use_usp:
            self.self_attn = SelfAttentionUSP(dim, num_heads, eps)
        else:
...
class Head(nn.Module):
    ...
    def forward(self, x, t_mod):
        ...
        return x
## borrowed from https://github.com/Wan-Video/Wan2.2/blob/main/wan/modules/s2v/model_s2v.py#L44
...
## borrowed from https://github.com/Wan-Video/Wan2.2/blob/main/wan/modules/s2v/audio_utils.py#L53
            ...
            if isinstance(mod, DiTBlock):
                for inject_id in inject_layer:
                    if f'root.transformer_blocks.{inject_id}' == mod_name:
                        logger.info(f"inject layer {mod_name} for cross attn")
...
## encode global music feature
## borrowed from https://github.com/li-ronghui/LODGE/blob/main/dld/models/architectures/model.py#L40
...
class WanModel(torch.nn.Module):
    def __init__(
        ...
        if enable_refimagevae:
            self.patch_embedding_refimagevae = nn.Sequential(
        ...
        self.blocks = nn.ModuleList([
            DiTBlock(has_image_input, dim, num_heads, ffn_dim, eps, use_usp=use_usp)
        ...
        if enable_dynamicfps or enable_unimodel:
        ...
        if enable_global or enable_dynamicfps or enable_unimodel:
            ...
            nhead = 4
            activation = F.gelu
            rotary = RotaryEmbedding(dim=latent_dim)
        ...
        if enable_music_inject:
            ...
            self.music_injector = Injector(all_modules, all_modules_names, dim=dim, num_heads=num_heads,
                                                inject_layer=music_inject_layers)

        self.enable_music_inject = enable_music_inject
        self.use_usp = use_usp
        self.enable_refimage = enable_refimage
        self.enable_refimagevae = enable_refimagevae
        self.enable_refface = enable_refface
        self.enable_dynamicfps = enable_dynamicfps
        self.enable_unimodel = enable_unimodel
    ...
    def patchify_global(self, x: torch.Tensor,control_camera_latents_input: torch.Tensor = None):
    ...
    def patchify_refimagevae(self, x: torch.Tensor):
        x = self.patch_embedding_refimagevae(x) 
        x = rearrange(x, 'b c f h w -> b f (h w) c').contiguous() # b t n c 和后续注入一致
        return x
    ...
    def forward(self,
        ...
        if enable_music_inject:
            adaptive_pool = nn.AdaptiveAvgPool2d((21, 1536))
            music_feature = adaptive_pool(music_feature)
            self.merged_audio_emb = music_feature
        ...
        for idx, block in enumerate(self.blocks):
            ...
            x = self.after_transformer_block(idx, x) ## add music injector
...
class WanModelStateDictConverter:
    ...
    def from_civitai(self, state_dict, dit_model_type=0):
